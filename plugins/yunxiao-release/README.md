...
```bash
curl -fsSL https://raw.githubusercontent.com/FlyAboveGrass/yunxiao-release-plugin/main/install-claude.sh | bash
```
...
Claude Code 安装脚本使用用户级插件作用域：已安装时只更新。脚本随后打开原生 `/plugin configure yunxiao-release@yunxiao-release-community`；已有 Token 会保留，缺失时由 Claude Code 隐藏读取并安全保存。退出该会话后，脚本再启动 `/yunxiao-release:yunxiao-release-config` 配置成员身份。
...
成员身份支持两种存储：项目内 `.codex/yunxiao-release.local.json`，或所有 worktree 共用的用户级 `${CODEX_HOME:-$HOME/.codex}/.env`。默认路径可由 Codex 和 Claude Code 的插件脚本直接读写；自定义 `CODEX_HOME` 时两个宿主必须使用同一值。项目配置存在时优先使用。
