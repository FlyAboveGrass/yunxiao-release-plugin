...
若项目原本已整体忽略 `.agents/`，脚本会写入最小的重新包含规则，只放行 `.agents/yunxiao-release.json`，本地身份和运行状态仍保持忽略。
...
只需编辑 `.agents/yunxiao-release.json`。每个字段的含义、默认行为和来源如下：
...
成员身份支持两种存储：项目内 `.agents/yunxiao-release.local.json`，或所有 worktree 共用的用户级 `${XDG_CONFIG_HOME:-$HOME/.config}/yunxiao-release/member.json`。两种路径都与宿主无关，可由 Codex 和 Claude Code 共用；项目配置存在时优先使用。
...
1. 询问存储范围：`项目` 或 `用户级`。
2. 询问用户名称（真实名字）和用户 ID。
3. 通过云效官方 MCP 核对输入 ID 与当前 Token 用户完全一致，并验证组织和代码库可见性。
4. 项目模式写入忽略的本地 JSON；用户级模式写入权限为 `600` 的用户配置 JSON。
...
用户级模式使用相同字段，写入 `${XDG_CONFIG_HOME:-$HOME/.config}/yunxiao-release/member.json`：
...
旧 Codex `.env` 中的 `YUNXIAO_DISPLAY_NAME`、`YUNXIAO_USER_ID` 仅作为迁移期兼容读取，不再写入。

字段说明：

| 字段 | 必填 | 说明 |
|---|---|---|
| `displayName` | 是 | 用户名称（真实名字），由成员输入；不参与认证。 |
...
| 文件 | Git | 用途 |
|---|---|---|
| `.agents/yunxiao-release.json` | 提交 | Codex 与 Claude Code 共用的团队配置 |
...
| `${XDG_CONFIG_HOME:-$HOME/.config}/yunxiao-release/member.json` | 不在项目中 | Codex 与 Claude Code 共用的用户级成员身份 |
| `${CODEX_HOME:-$HOME/.codex}/.env` | 不在项目中 | 仅供 Codex 注入 `YUNXIAO_ACCESS_TOKEN` |
| Claude Code 敏感 `userConfig` | 不在项目中 | 仅供 Claude Code 注入 `yunxiao_access_token` |
...
Codex 当前安装仍是用户级：插件对该用户的其他项目可用，脚本只在当前 Git 项目生成 `.agents/yunxiao-release.json` 等项目配置。
...
操作后重启对应宿主。卸载不会删除项目的 `.agents/yunxiao-release.json`；用户级成员配置、Codex Home Token 和 Claude Code 敏感配置也不会被自动删除。
...
node bin/yunxiao-release.mjs --help
