...
- `ask`：创建 MR 后询问是否同步和处理评论；合并前准备时重新确认，允许选择跳过。
...
创建 MR 前必须验证配置的 remote 和目标分支，并使用完整 refspec `git fetch <remote> +refs/heads/<target>:refs/remotes/<remote>/<target>` 刷新远端目标分支，禁止自行缩写。若远端目标分支不是当前分支祖先，先获得单独确认并普通合入；未完成合入不得继续推送或创建 MR。若当前分支已有开启中的 MR，停止并提示已有记录，不恢复或重复创建。
