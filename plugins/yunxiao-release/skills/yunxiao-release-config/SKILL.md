...
7. 检查项目成员配置和 Codex Home `.env` 成员配置；项目配置存在时优先使用。
8. 初始化或更新成员配置时，交互询问存储范围（`项目` 或 `Codex Home`）、成员显示名称和用户 ID，不要求输入 `tokenSource`。
...
12. 选择项目存储时，将 `displayName`、`userId` 写入 `localConfigFile` 并用 `git check-ignore` 验证；选择 Codex Home 时，通过 stdin 把 `{displayName, userId}` JSON 交给 `scripts/configure-member.mjs` 安全更新 `.env`，禁止把用户输入拼入 shell 命令。如果项目成员配置已存在，说明它会覆盖 Home 配置，经用户确认后删除项目文件。
