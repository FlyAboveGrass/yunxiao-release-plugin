...
description: 初始化、更新或检查任意 Git 项目的云效发版配置和成员身份。用户提到初始化云效、配置 Token、检查 MCP 认证、切换成员、目标分支或发版配置缺失时使用。
---
...
3. 展示共享配置，确认组织 ID、仓库 ID、remote、目标分支和可选发版文件已填写；目标分支未修改时使用 `master`。
4. 检查成员本地配置；缺失时询问成员显示名称。
5. 确认当前会话真实存在云效官方 MCP 工具，并读取其 Schema。
6. 调用当前用户、组织、仓库和目标分支的只读工具验证认证与配置。
7. 将 MCP 返回的非敏感 `userId`、成员显示名称和 `tokenSource: environment` 写入配置指定的本地文件。
8. 使用 `git check-ignore` 验证成员配置和运行目录已被忽略。
9. 输出成员名称、MCP 用户、组织、仓库、remote、目标分支、Review 模式、可选发版文件及权限验证结果。
